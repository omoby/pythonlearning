...
from meituanwaimai.meituan.Ua_cookie import MeiTuanSpiser, COOKIE
from meituanwaimai.meituan.parse_woff import parseWoff, parseString
...
class WaiMai(object):
    ...
    def read_csv(self):
        ...
        """
        try:
            with open('city_list.csv', encoding='utf-8') as csvfile:
                            ...
                            try:
                                ...
                                if 'failed' == shop_list:
                                    ...
                                    for shop_l in shop_list:
                                        # print(shop_l)
                                        shop_text = parseString(json.dumps(shop_l), parseDict)
                                        shop_json = json.loads(shop_text)
                                        # print(shop_json)
                                        self.insert_mysql(city_name=city_name, lng=long, lat=lat, shop_l=shop_json)
                                        # print(shop_l)
                                        # if insert_flag == 'pass':
                                        #     continue
    ...
    def insert_mysql(self, city_name, lng, lat, shop_l):
        """
        ...
        data = (
            city_name, lng, lat, shop_id, shop_name, month_sales, shop_pic_url, shop_score, delivery_time,
            start_price, shipping_send_price,
            average_price, shipping_time_info, distance, address)
        # print(data)
        sql = 'insert into wm_shop(city_name,lng, lat, shop_id,shop_name,month_sales,shop_pic_url,' \
              'shop_score,delivery_time,start_price,shipping_send_price,' \
              'average_price,shipping_time_info,distance,address) values ("%s","%s","%s","%s"' \
