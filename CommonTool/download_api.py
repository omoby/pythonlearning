...
from shanghai_chinatax.date_util import get_date, get_timestamp
from shanghai_chinatax.encrypt_util import get_secret
...
class DownloadApiUtil():
    ...
    def get_url(self, task_id, task_intance, data_size):
        ...
        :param task_id: 任务id
        :param task_intance: 实例id
        :param data_size: 获取数据的size
        :return: 请求的数据
        '''

        date = get_date()
        aut = 'hmac username="' + user + '", algorithm="hmac-sha256", headers="date", signature="' + get_secret(
            date) + '"'
        headers = {
            'Date': date,
            'Authorization': aut,
        ...
        # 向服务器发送请求
        method = "GET"
        request_url = "http://172.22.5.243:30080/download-api/v1/requests/" + task_id + "/" + task_intance + "?user=" + user + "&size=" + str(
            data_size) + "&ts=" + str(get_timestamp())
        # print(url)
